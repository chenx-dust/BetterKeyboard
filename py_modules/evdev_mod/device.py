...
class InputDevice(EventIO, Generic[_AnyStr]): # type: ignore
    """
    ...
    def __init__(self, dev: str | bytes | os.PathLike[str] | os.PathLike[bytes]):
        """
        Arguments
        ---------
        ...
        #: Path to input device.
        self.path: _AnyStr = dev if not hasattr(dev, "__fspath__") else dev.__fspath__() # type: ignore
        ...
        info_res = input.ioctl_devinfo(self.fd)
        ...
        self.version: int = input.ioctl_EVIOCGVERSION(self.fd)

        #: The raw dictionary of device capabilities - see `:func:capabilities()`.
        self._rawcapabilities = input.ioctl_capabilities(self.fd)

        #: The number of force feedback effects the device can keep in its memory.
        self.ff_effects_count = input.ioctl_EVIOCGEFFECTS(self.fd)
    ...
    def capabilities(self, verbose: bool = False, absinfo: bool = True) -> Union[Dict[int, List[int]], Dict[Tuple[str, int], List[Tuple[str, int]]]]:
        ...
        if verbose:
            return dict(util.resolve_ecodes_dict(self._capabilities(absinfo))) # type: ignore
        else:
    ...
    def input_props(self, verbose: bool = False):
        ...
        """
        props = input.ioctl_EVIOCGPROP(self.fd)
        if verbose:
    ...
    def leds(self, verbose: bool = False):
        ...
        """
        leds = input.ioctl_EVIOCG_bits(self.fd, ecodes.EV_LED)
        if verbose:
    ...
    def grab(self) -> None:
        ...
        input.ioctl_EVIOCGRAB(self.fd, 1)
    ...
    def ungrab(self) -> None:
        ...
        input.ioctl_EVIOCGRAB(self.fd, 0)
    ...
    def repeat(self):
        ...
        return KbdInfo(*input.ioctl_EVIOCGREP(self.fd))

    @repeat.setter
    def repeat(self, value: Tuple[int, int]):
        return input.ioctl_EVIOCSREP(self.fd, *value)
    ...
    def active_keys(self, verbose: bool = False):
        ...
        """
        active_keys = input.ioctl_EVIOCG_bits(self.fd, ecodes.EV_KEY)
        if verbose:
    ...
    def absinfo(self, axis_num: int):
        ...
        """
        return AbsInfo(*input.ioctl_EVIOCGABS(self.fd, axis_num))
    ...
    def set_absinfo(self, axis_num: int, value=None, min=None, max=None, fuzz=None, flat=None, resolution=None) -> None:
        ...
        input.ioctl_EVIOCSABS(self.fd, axis_num, new_absinfo)
